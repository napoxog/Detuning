...
def spl_int(x,y,x_out,y_out,step):
    ...
    if n == 2:
         ...
         if  n != 3 :
               ...
               c[n-1]=-(c[n-2]/(x[n-1]-x[n-3])-c[n-3]/(x[n-2]-x[n-4]))*d[n-2]*d[n-2]/(x[n-1]-x[n-4]);
         for i in range(1,n):
...
while dtidx>=0:
    ...
    if tuning_values[dtidx][0]!=0:
        tuning_values[dtidx]=(tuning_values[dtidx][0],tuning_values[dtidx][1],notune,notune/tuning_values[dtidx][0])
    else :
        tuning_values[dtidx]=(tuning_values[dtidx][0],tuning_values[dtidx][1],notune,0)

    if dtidx :
        print ('{0:10}  {1:20} {2:20} {3:20} {4:20} '.format(dt,camp,notune,notune/camp,(tuning_values[len(tuning_values)-1][1])*notune/camp))
    else:
        print ('{0:10}  {1:20} {2:20} {3:20} {4:20} '.format(dt,camp,notune,prev_notune/prev_camp,prev_notune/prev_camp*tuning_values[len(tuning_values)-1][1]))
    dt=dt-w.SampleRate_interp
    prev_camp=camp
    prev_notune=notune
    dtidx=dtidx-1
